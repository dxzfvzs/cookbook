...
function isDrink(recipe: Recipe) {
  return recipe.categories?.includes("drink") ?? false
}
...
export function filterVisibleCategories(recipe: Recipe, activeCategories: CategoryId[]): CategoryId[] {
  ...
  return recipe.categories
    ...
    .filter((category) => activeCategories.includes(category) || !(isDrink(recipe) && (categoryById[category].group === "type" || categoryById[category].group === "diet")))
